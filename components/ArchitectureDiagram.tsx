...
const Node: React.FC<{
  ...
  <div
    className={`p-4 rounded-xl glass-card border-l-4 ${color} flex flex-col items-center text-center w-full max-w-[180px]`}
  >
      ...
      <div className="mt-12 p-6 bg-white/5 rounded-2xl border border-white/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-gray-400 font-mono">
          <div className="p-3 bg-black/20 rounded-lg">
            <span className="text-violet-300">Aşama 1:</span>{" "}
            <code>X-Tenant-ID</code> başlığına göre Bun:SQL üzerinden dinamik
            bağlantı havuzu yönetimi.
          </div>
          <div className="p-3 bg-black/20 rounded-lg">
            <span className="text-violet-300">Aşama 2:</span> Drizzle session
            context'i kullanılarak Postgres seviyesinde RLS (Row Level Security)
            zorlaması.
          </div>
          <div className="p-3 bg-black/20 rounded-lg">
            <span className="text-violet-300">Aşama 3:</span> Hono middleware'i,
            işlem yürütülmeden önce kiracı abonelik limitlerini doğrular.
          </div>
          <div className="p-3 bg-black/20 rounded-lg">
            <span className="text-violet-300">Aşama 4:</span> Edge
            lokasyonlarında Valkey (Redis halefi) aracılığıyla dağıtık
            önbellekleme.
          </div>
        </div>
      </div>
